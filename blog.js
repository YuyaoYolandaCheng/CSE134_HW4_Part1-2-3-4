...
if(window.localStorage.length > 0){
    for (var key in window.localStorage) {
        ...
        old_add.insertCell(3).innerHTML = `<button onclick='myEdit(this)'>edit</button>
...
add.onclick = function(){
    ...
    ok_p1.onclick = function(){
        ...
        new_add.insertCell(3).innerHTML = `<button onclick='myEdit(this)'>edit</button>
                            <button onclick='myDelete(this)'>delete</button>`;
...
function myDelete(button){
    ...
    okConfirm.onclick = function() {
        row = button.parentNode.parentNode; 
        document.getElementById('crudlist').deleteRow(row.rowIndex);
